...
// LoadToken reads the API token from the MINT_API_TOKEN environment variable.
...
func LoadToken() (string, error) {
	if token := os.Getenv("MINT_API_TOKEN"); token != "" {
		return token, nil
	}

	home, err := os.UserHomeDir()
	if err != nil {
		return "", fmt.Errorf("no API token found. Set MINT_API_TOKEN or run 'mint login'")
	}
