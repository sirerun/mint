...
- Where does the infrastructure run? Sire-managed GCP/AWS accounts.
...
- What is the trust boundary? Users upload generated server code; Sire builds and runs it.

## Decision
Build a managed hosting platform under `mint deploy managed` that:
1. Accepts generated server source via `--source` flag.
2. Builds the container using Sire-managed infrastructure (Cloud Build or CodeBuild).
3. Deploys to a multi-tenant container platform with per-server isolation.
4. Assigns a `{service}.mcp.sire.run` subdomain with managed TLS.
...
The backend API runs as a Sire service (separate repo). The mint CLI sends the source tarball to the API, which handles build, deploy, DNS, and metering. The CLI implementation in `internal/deploy/managed/` is a thin client that calls the Sire hosting API.
...
- Trust boundary: Sire runs user-generated code. Requires sandboxing and security controls.
